...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE tb_cliente SET cpf_cliente = ?, nm_cliente = ?, nm_email_cliente = ?, 
                              nr_telefone_cliente = ?, nm_endereco_cliente = ?, nr_endereco_cliente = ? 
                              WHERE id_cliente = ?");
        $stmt->execute([$cpf, $nome, $email, $telefone, $endereco, $numero, $id]);
...
<title>Editar Cliente - Depósito</title>
...
<header class="mb-8">
<h1 class="text-3xl font-bold text-primary mb-2">Editar Cliente</h1>
...
<input type="text" name="nome" value="<?= $cliente['nm_cliente'] ?>" required 
...
<input type="text" name="cpf" value="<?= $cliente['cpf_cliente'] ?>" required 
                               class="w-full bg-gray-700 border border-gray-600 rounded py-2 px-3 text-white">
</div>
 
                    <div class="mb-4">
<label class="block text-gray-400 mb-2">Email</label>
<input type="email" name="email" value="<?= $cliente['nm_email_cliente'] ?>" required 
                               class="w-full bg-gray-700 border border-gray-600 rounded py-2 px-3 text-white">
</div>
 
                    <div class="mb-4">
<label class="block text-gray-400 mb-2">Telefone</label>
<input type="text" name="telefone" value="<?= $cliente['nr_telefone_cliente'] ?>" required 
                               class="w-full bg-gray-700 border border-gray-600 rounded py-2 px-3 text-white">
</div>
...
<input type="text" name="endereco" value="<?= $cliente['nm_endereco_cliente'] ?>" 
                                   placeholder="Rua/Avenida" required 
                                   class="flex-1 bg-gray-700 border border-gray-600 rounded py-2 px-3 text-white">
<input type="number" name="numero" value="<?= $cliente['nr_endereco_cliente'] ?>" 
